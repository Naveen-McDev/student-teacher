...
export function EditTeacher() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            console.log(values)
            try {
                await axios.put(`https://62de2a29ccdf9f7ec2d2197a.mockapi.io/teachers/${params.id}`, values)
    ...
    let teachersData = async () => {
        try {
            let userdata = await axios.get(`https://62de2a29ccdf9f7ec2d2197a.mockapi.io/teachers/${params.id}`);
    ...
    return (
        <>
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800">Edit Teacher ID: {params.id}</h1></div>
